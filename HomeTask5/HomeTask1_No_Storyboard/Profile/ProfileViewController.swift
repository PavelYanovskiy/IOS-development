...
class ProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // Передаём в функцию getProfileInfo замыкание, которое в самой функции обозначим как @escaping, чтобы после выполнения функции сюда вернулся массив, который мы сможем положить в model.
        networkService.getProfileInfo { [weak self] (profile: Profile) in
    ...
    func updateProfile(profile: Profile) {
        ...
        // Поскольку мы будем получать данные по url (через try? Data), мы должны предварительно вручную встать на другой поток (в глобальную очередь), а потом также вручную вернуться обратно на главный поток (в основную очередь)
